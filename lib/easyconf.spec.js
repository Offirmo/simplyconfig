	...
	describe('write', function () {
		...
		describe('add()', function () {
			...
			describe('with env', function () {
				...
				it('should add env vars to the config root', function () {
					var config = easyconf
						.create()
						.add('env');

					expect(config.get('PATH')).to.exist;
					expect(config.get('NODEJS__server__port')).to.equal('1234');
					expect(config.get('FOO_API_KEY')).to.equal('27A13');
				...
				it('should allow to place loaded env vars', function () {
					...
					expect(config.get('NODEJS__server__port')).to.not.exist;
					...
					expect(config.get('foo:bar:NODEJS__server__port')).to.equal('1234');
					expect(config.get('foo:bar:FOO_API_KEY')).to.equal('27A13');
				...
				describe.only('expansion of specially formatted env vars to proper config values', function () {
