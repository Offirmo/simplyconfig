...
describe('easyconf', function () {
	...
	describe('read = get()', function () {
		...
		it('should return a copy of the data, to protect config against modifications', function () {
			var test_data = {
				foo: {
					bar: 42
				}
			};
			var config1 = easyconf
				.create()
				.add(test_data);
			var config2 = easyconf
				.create()
				.add(test_data);
			var config3 = easyconf
				.create()
				.add(config1);

			expect(config1.get('foo:bar')).to.equal(42);
			var obj1 = config1.get('foo');
			obj1.bar = 43;
			expect(config1.get('foo:bar')).to.equal(42); // no change
			expect(config2.get('foo:bar')).to.equal(42); // no change either
			expect(config3.get('foo:bar')).to.equal(42); // no change either
		});

		context('with no key', function () {
		...
		context('with a simple key', function () {
		...
		context('with a deep key', function () {
	...
	describe('write', function () {
		describe('add()', function () {
			...
			context('with files', function () {
				...
				context('with config as .json', function () {
			...
			describe('values expansion', function () {
		...
		describe('set()', function () {
			...
			it('should add a deep key', function () {
				var config = easyconf
					.create()
					.add({
						key1: {
				...
				config.set('deep:very_deep:foo', 'bar');
